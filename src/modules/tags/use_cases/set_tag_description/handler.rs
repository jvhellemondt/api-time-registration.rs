...
use crate::modules::tags::use_cases::set_tag_description::decide::decide_set_description;
...
#[derive(Debug, Clone)]
pub struct SetTagDescriptionHandler<TEventStore>
where
...
impl<TEventStore> SetTagDescriptionHandler<TEventStore>
where
    TEventStore: EventStore<TagEvent> + Send + Sync + 'static,
{
    ...
    pub async fn handle(
        ...
        match decide_set_description(&state, command) {
...
#[cfg(test)]
mod set_tag_description_handler_tests {
    use super::*;
    ...
    #[rstest]
    #[tokio::test]
    async fn handle_set_description_appends_event(cmd: SetTagDescription) {
        let event_store = InMemoryEventStore::<TagEvent>::new();
        create_tag(event_store.clone()).await;
        let handler = SetTagDescriptionHandler::new(event_store.clone());
    ...
    #[rstest]
    #[tokio::test]
    async fn handle_set_description_fails_if_tag_not_found(cmd: SetTagDescription) {
        let result = SetTagDescriptionHandler::new(InMemoryEventStore::<TagEvent>::new())
    ...
    #[rstest]
    #[tokio::test]
    async fn handle_set_description_fails_if_tag_deleted(cmd: SetTagDescription) {
        ...
        DeleteTagHandler::new(event_store.clone())
            ...
            .await
            .unwrap();
        let result = SetTagDescriptionHandler::new(event_store)
            .handle(STREAM_ID, cmd)
            .await;
        assert!(matches!(
            result,
    ...
    #[rstest]
    #[tokio::test]
    async fn handle_set_description_fails_if_event_store_is_offline(cmd: SetTagDescription) {
        let event_store = InMemoryEventStore::<TagEvent>::new();
    ...
    #[rstest]
    #[tokio::test]
    async fn handle_set_description_fails_on_version_conflict(cmd: SetTagDescription) {
        use tokio::join;
        let event_store = InMemoryEventStore::<TagEvent>::new();
        create_tag(event_store.clone()).await;
        event_store.set_delay_append_ms(10);
        let h1 = SetTagDescriptionHandler::new(event_store.clone());
        let h2 = SetTagDescriptionHandler::new(event_store);
