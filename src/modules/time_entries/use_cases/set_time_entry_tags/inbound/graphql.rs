...
mod set_time_entry_tags_graphql_inbound_tests {
    ...
    async fn returns_true_on_valid_input() {
        ...
        let result = schema
            .execute(format!(
                r#"mutation {{ setTimeEntryTags(timeEntryId: "{te_id}", userId: "u-1", tagIds: ["tag-1", "tag-2"]) }}"#
            ))
            .await;
    ...
    async fn returns_true_on_empty_tags() {
        ...
        let result = schema
            .execute(format!(
                r#"mutation {{ setTimeEntryTags(timeEntryId: "{te_id}", userId: "u-1", tagIds: []) }}"#
            ))
            .await;
        assert!(result.errors.is_empty());
        assert_eq!(result.data.to_string(), "{setTimeEntryTags: true}");
    ...
    async fn returns_error_on_non_v7_uuid() {
        ...
        let result = schema
            .execute(format!(
                r#"mutation {{ setTimeEntryTags(timeEntryId: "{v4_id}", userId: "u-1", tagIds: ["tag-1"]) }}"#
            ))
            .await;
        assert!(!result.errors.is_empty());
    }
    ...
    async fn returns_error_when_event_store_offline() {
        ...
        let result = schema
            .execute(format!(
                r#"mutation {{ setTimeEntryTags(timeEntryId: "{te_id}", userId: "u-1", tagIds: ["tag-1"]) }}"#
            ))
            .await;
        assert!(!result.errors.is_empty());
    }
